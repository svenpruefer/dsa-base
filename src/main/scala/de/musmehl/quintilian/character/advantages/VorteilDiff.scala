...
final case class VorteilDiff(add: Set[Vorteil] = Set.empty, remove: Set[Vorteil] = Set.empty) {
  def applyTo(vorteile: Set[Vorteil]): Set[Vorteil] =
    vorteile.union(add).diff(remove)
}

object VorteilDiff {
  val empty: VorteilDiff = VorteilDiff()
}
