...
final case class NachteilDiff(add: Set[Nachteil] = Set.empty, remove: Set[Nachteil] = Set.empty) {
  def applyTo(Nachteile: Set[Nachteil]): Set[Nachteil] =
    Nachteile.union(add).diff(remove)
}

object NachteilDiff {
  val empty: NachteilDiff = NachteilDiff()
}
